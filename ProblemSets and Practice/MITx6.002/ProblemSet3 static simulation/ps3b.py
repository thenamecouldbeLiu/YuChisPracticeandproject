...
class SimpleVirus(object):
    ...
    def doesClear(self):
        ...
        False.
        """
        totest = random.random()
        if totest <= self.getClearProb():
            return True
        else:
            return False
    ...
    def reproduce(self, popDensity):
        ...
        if_reproduce = random.random()
        if if_reproduce<= self.maxBirthProb*(1 - popDensity):
...
class Patient(object):
    ...
    def update(self):
        ...
        for v in self.viruses:
            try: 
                if self.maxPop> self.getTotalPop():
                    self.viruses.append(v.reproduce(current_popDensity))
...
def simulationWithoutDrug(numViruses, maxPop, maxBirthProb, clearProb,
    ...
    """
    resultdict= dict(((x,y), 0) for x in range(1,numTrials+1) for y in range(1,301))
    ...
    for trial in range(1,numTrials+1):
        ...
        new_patient = Patient(viruslist, maxPop)
        for time in range(1,301):
            resultdict[trial,time] = new_patient.update()
    for t in range(1,301):
        magicnum = 0
        for i in range(1, numTrials+1):
            magicnum += resultdict[i, t]
        yaxislist.append(magicnum/numTrials)

    ploty = yaxislist
    pylab.plot(ploty, label = "SimpleVirus")
    ...
    def reproduce(self, popDensity, activeDrugs):
        ...
        if all(self.isResistantTo(drug) for drug in activeDrugs)==True:
            if random.random() <=self.maxBirthProb * (1 - popDensity):
                ...
                for item in self.resistances.keys():
                    mutateprob = random.random()
                    
                    if mutateprob <= self.mutProb:
                        if self.resistances[item]:
                            New_resistance[item] = False
                        else:
                            New_resistance[item] =True
...
#        print(viruses_copy)
        ...
        CurrentDensity = len(self.viruses)/self.maxPop
        viruses_copy2 = self.getViruses()[:]
#        print(viruses_copy)
        for v in viruses_copy2:
            try:
                self.viruses.append(v.reproduce(CurrentDensity, self.Prescription))
            except NoChildException:
                continue
        return len(self.viruses)
...
def simulationWithDrug(numViruses, maxPop, maxBirthProb, clearProb, resistances,
    ...
    for trial in range(numTrials):
        ...
        new_Treatedpatient = TreatedPatient(viruslist, maxPop)
        
        for time in range(timeSteps):
#            print(new_Treatedpatient.getTotalPop())
            new_Treatedpatient.update()
            
#            print(patient.getTotalPop(), end=" ")
            eachRunValuesTotal.append(new_Treatedpatient.getTotalPop())
            eachRunValuesGResistant.append(new_Treatedpatient.getResistPop(['guttagonol']))
            if time == 149:
                new_Treatedpatient.addPrescription("guttagonol")
