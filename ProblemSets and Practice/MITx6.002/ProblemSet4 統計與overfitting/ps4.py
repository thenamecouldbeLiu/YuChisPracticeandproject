...
def evaluate_models_on_training(x, y, models):
    ...
    Returns:
        None
    """
    def est_result(x, model):
        model_est = []
        for xval in x:
            estnumber = 0
            coepower = len(model) - 1
            for polyco in model:
                estnumber+= polyco*xval**coepower
                coepower -= 1
                
            model_est.append(estnumber)
        return model_est
    for model in models:
        estlist = est_result(x,model)
        estlist =np.array(estlist)
        Rsqare = r_squared(y, estlist)
        pylab.plot(x, y, ":b", label = "origin")
        pylab.plot(x, estlist, "-r", label = "Estimate"+",R2="+str(Rsqare))
        pylab.title("Original and Estimated Graph")
        pylab.xlabel('years')
        pylab.ylabel('Temperatures change')
        pylab.legend()
...
for year in x1:
    YearlyTemperature =raw_data.get_yearly_temp('BOSTON', year)
    AverageTemperature = np.mean(YearlyTemperature)
    y.append(AverageTemperature)
models = generate_models(x, y, [1])    
evaluate_models_on_training(x, y, models)
