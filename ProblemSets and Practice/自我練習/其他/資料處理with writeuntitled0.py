# -*- coding: utf-8 -*-
"""
Created on Sun Jun 10 23:12:32 2018
...
class static_describe(object):
    ...
    def findStatic(self, save_txt=None):
        ...
        self.emotion_list =['中立', '正面','負面']
        '''點閱數Filter'''
        self.non_filted= selected_df[selected_df['點閱數']>=0]
        ...
        '''filter名稱與字典'''
        self.filted_name= ["總平均點擊率:",'點閱數等於0:', '點閱數大於0 :','點閱數大於0 小於1000:','點閱數大於1000:','點閱數大於5000:']
    ...
    def findStaticAndWrite(self, save_txt="NewStaticText.txt"):
        ...
        self.filted_list=[self.non_filted, self.filter_click_quales_0,self.filter_click_bigger_than0,self.filter_click_bigger0_smaller1000, self.filter_click_bigger_than1000, self.filter_click_bigger_than5000,]
        self.filter_dict = dict(zip(self.filted_name, self.filted_list))
        #f.write('點閱數大於5000:',len(filter_click_bigger_than5000))
        #f.write('點閱數大於1000:',len(filter_click_bigger_than1000))
        #f.write('點閱數大於0 小於1000:',len(filter_click_bigger0_smaller1000))
        #f.write('點閱數大於0 :',len(filter_click_bigger_than0))
        #f.write('點閱數等於:0:',len(filter_click_quales_0))
        '''算平均與標準差'''
        for file in self.filter_dict:
            f.writelines(["-"*20+"\n",str(file),'\n'*2,"限制範圍樣本數:",str(len(self.filter_dict[file])),'\n'*2])
            f.writelines(["平均值:",'\n',str(self.filter_dict[file].mean()),'\n'*2])
            f.writelines(["標準差:",'\n',str(self.filter_dict[file].std()),'\n'*2])
            f.write("-"*20+'\n')
        
        f.writelines(["情緒強度",'\n'*2])
        for emo in self.emotion_list:
            filter_emo=selected_df['情緒標記']==emo
            f.writelines([emo,"強度個數:",str(len(selected_df[filter_emo])),"\n"*2])
        f.writelines(["-"*20,'\n'*2])
        for emo in self.emotion_list:
            try:
                f.writelines(["總體平均",emo,"強度:", str((selected_df[emo+'強度']).mean()),"\n"*2])
            except:
                pass
        ...
        for source in source_set:
            ...
            for emo in self.emotion_list:
                f.writelines([emo,":",str(len(source_mark[(source_mark["來源"]==source) & (source_mark["情緒標記"]==emo)])),"\n"*2])
        f.close()
...
#        字典的值須為Tuple才能正常繪圖
        ...
        return emotion_dict
    def getDiscussionSourceSumDict(self,sources=['討論區','社群網站'], click=None):
        '''比較來源'''
        df = self.file_df
        source_web_dict = {}
        for source in sources:
            source_web_dict[source] =len(df[df['來源']==source])
        return source_web_dict
    def getDiscussionSourceWebDict(self,sources, click=None):
        '''比較來源-->來源網站'''
        df = self.file_df
        source_web_dict = {}
        selected_df = df[df['來源']==sources]
        if sources=="All":
            selected_df=df
        for source in set(df['來源網站']):
            source_web_dict[source] =len(selected_df[selected_df['來源網站']==source])
        return source_web_dict
...
def pieGraphPlot(data,xlabel=None,ylabel=None, graphTitle=None,figureName = None, save_graph =False):
    ...
    plt.title(graphTitle)
    plt.pie(x= data.values(),labels=data.keys(),autopct='%1.2f%%',pctdistance=0.8 ,radius = 1.2,textprops = {'fontsize':18, 'color':'k'})
    if figureName!= None and save_graph:
        plt.savefig(figureName)
    plt.show()
...
#    testClass.findStaticAndWrite(save_txt=data+"資料文字分析.txt")
#    pieGraphPlot(testClass.getDiscussionSourceSumDict(),graphTitle=data +"來源比較圓餅圖"+"\n",figureName=data+"來源比較"+'圓餅圖.jpg' ,save_graph=True)
    for item in ["All","討論區", "社群網站"]:
        pieGraphPlot(testClass.getDiscussionSourceWebDict(item),graphTitle=data+item +"組成圓餅圖"+"\n",figureName=data+item+'資料來源組成圓餅圖.jpg' ,save_graph=True)
#        barGraphPlot(testClass.getEomtionNumberDict(item), "情緒", "總和", graphTitle= (data+" "+item+"總和長條圖"), figureName=data+item+"總和直條圖.jpg",save_graph=True)
#        pieGraphPlot(data = testClass.getEomtionNumberDict(item),graphTitle=data+item+ "情緒分布圓餅圖"+"\n",figureName=data+item+'圓餅圖.jpg' ,save_graph=True)
#        barGraphPlot(testClass.getEomtionAverageDict(item), "情緒", "平均值", graphTitle=(data+" "+item+" "+"平均值"),figureName=data+item+"情緒強度平均直條圖.jpg",save_graph=True)
#        barGraphPlot(testClass.getEomtionStdDict(item), "情緒", "標準差", graphTitle=data+" "+item+" "+"標準差",figureName=data+item+"情緒強度標準差直條圖.jpg",save_graph=True)
#barGraphPlot(testClass.getCampareSumDict(select_filter="All"),"情緒", "比較數字", graphTitle=("資料總和比較直條圖"), figureName="資料總和比較直條圖.jpg",save_graph=True)
#barGraphPlot(testClass.getCompareMeanDict(select_filter="All"),"情緒", "比較數字", graphTitle=("資料平均比較直條圖"), figureName="資料平均比較直條圖.jpg",save_graph=True)
#barGraphPlot(testClass.getCompareStdDict(select_filter="All"),"情緒", "比較數字", graphTitle=("資料標準差比較直條圖"), figureName="資料標準差比較直條圖.jpg",save_graph=True)
